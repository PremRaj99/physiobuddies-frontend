...
export default function TherapistProfilePage() {
  ...
  const { data: detailData } = useTherapistDetail(therapistId);
  const { data: reviewsData } = useTherapistReviews(therapistId);
  const { data: articlesData } = useTherapistArticlesById(therapistId);
  const { data: faqsData } = useTherapistFaqsById(therapistId);

  const updateInfo = useUpdateUserInfo();

  const detail = detailData?.data;

  // Merge live user + therapist detail into the profile shape the UI expects.
  const profile: TherapistProfile = {
    name: user?.name ?? '',
    email: user?.email ?? '',
    phone: user?.phone ?? '',
    image: user?.image ?? '',
    displayAddress: detail?.displayAddress ?? user?.therapistProfile?.displayAddress ?? '',
    about: detail?.about ?? user?.therapistProfile?.about ?? '',
    gender: 'other',
    dob: '',
    mode: 'home_visit',
    clinic: null,
    price: detail?.discountedPrice ?? 0,
    commissionRate: 0,
    rating: detail?.rating ?? 0,
    verifiedAt: user?.therapistStatus?.isVerified ? new Date().toISOString() : null,
    experience: detail?.experience ?? 0,
    specialization: detail?.specializations ?? [],
    languagesSpoken: [],
    educationQualification: [],
    currentlyAffiliation: null,
    IAPId: null,
    resume: null,
  };

  const articles: Article[] = (articlesData?.data ?? []).map((a, idx) => ({
    id: String(idx),
    title: a.title,
    content: a.content,
  }));

  const faqs: FAQ[] = (faqsData?.data ?? []).map((f, idx) => ({
    id: String(idx),
    question: f.question,
    answer: f.answer,
  }));

  const reviews: Review[] = (reviewsData?.data ?? []).map((r, idx) => ({
    id: String(idx),
  ...
  const handleSaveProfile = (updatedProfile: TherapistProfile) => {
    updateInfo.mutate({ name: updatedProfile.name, mobile: updatedProfile.phone });
  };
          ...
          <div className="min-w-0 flex-1">
            <AnimatePresence mode="wait">
              <TabsContent value="profile" className="mt-0 outline-none">
                <GeneralInfoTab
                  key={`${profile.name}-${profile.about}-${profile.displayAddress}`}
                  profile={profile}
                  onSave={handleSaveProfile}
                  isSaving={updateInfo.isPending}
                />
              </TabsContent>
              ...
              <TabsContent value="bank" className="mt-0 outline-none">
                <BankDetailsTab bankDetails={EMPTY_BANK} />
              </TabsContent>
              <TabsContent value="content" className="mt-0 outline-none">
                <ContentManagementTab articles={articles} faqs={faqs} />
              </TabsContent>
