...
// --- Types ---
type ActivityType = 'frequent' | 'likely' | 'possible' | 'rare' | 'unlikely';
...
const MOCK_ACTIVITIES: ActivityRecord[] = [
  {
    id: 'act_101',
    userId: 'usr_201',
    title: 'Slot Configuration Changed',
    data: 'Modified recurring morning slots schedule starting from 6:00 AM.',
    before: JSON.stringify({ slots: ['08:00 AM', '09:00 AM', '10:00 AM'], slotDuration: 60 }),
    after: JSON.stringify({
      slots: ['06:00 AM', '07:00 AM', '08:00 AM'],
      slotDuration: 40,
      breakDuration: 20,
    }),
    ip: '157.45.201.32',
    type: 'possible',
    createdAt: '2026-07-05T18:42:00.000Z',
  },
  {
    id: 'act_102',
    userId: 'usr_201',
    title: 'Therapist Profile Updated',
    data: 'Updated consultation prices and professional details.',
    before: JSON.stringify({ price: 1000, experience: 8, clinic: null }),
    after: JSON.stringify({ price: 1500, experience: 8, clinic: 'Trust Wellness Clinic' }),
    ip: '157.45.201.32',
    type: 'frequent',
    createdAt: '2026-07-05T17:15:30.000Z',
  },
  {
    id: 'act_103',
    userId: 'usr_201',
    title: 'Assessment Form Created',
    data: 'Generated initial physiotherapy assessment report for John Doe.',
    before: null,
    after: JSON.stringify({
      patientId: 'PAT-101',
    ...
    ip: '157.45.201.32',
    type: 'frequent',
    createdAt: '2026-07-05T14:38:09.000Z',
  },
  {
    id: 'act_104',
    userId: 'usr_305',
    title: 'Account Login',
    data: 'Successful authentication from new browser session.',
    before: null,
    after: JSON.stringify({ browser: 'Chrome', os: 'Windows 11', device: 'Desktop' }),
    ip: '192.168.1.105',
    type: 'likely',
    createdAt: '2026-07-05T12:00:15.000Z',
  },
  {
    id: 'act_105',
    userId: 'usr_201',
    title: 'Payout Details Updated',
    data: 'Modified bank account details for direct transfer payouts.',
    before: JSON.stringify({ bankName: 'HDFC Bank', accountNumber: '******4567' }),
    after: JSON.stringify({ bankName: 'ICICI Bank', accountNumber: '******7890' }),
    ip: '203.0.113.195',
    type: 'rare',
  ...
  },
  {
    id: 'act_106',
    userId: 'usr_999',
    title: 'Failed API Signature Validation',
    data: 'Blocked request with invalid JWT authorization token signature.',
    before: null,
    after: JSON.stringify({
      endpoint: '/api/v1/therapist/payouts',
      failureReason: 'Signature verification failed',
    }),
    ip: '198.51.100.72',
    type: 'unlikely',
    createdAt: '2026-07-03T23:12:44.000Z',
  },
];
...
export default function ActivityPage() {
  ...
  // Aggregate stats
  const stats = useMemo(() => {
    const uniqueIps = new Set(MOCK_ACTIVITIES.map((act) => act.ip)).size;
    const unusualEvents = MOCK_ACTIVITIES.filter(
    ...
    return {
      ...
      uniqueIps,
      recent: MOCK_ACTIVITIES.filter(
        (act) => new Date().getTime() - new Date(act.createdAt).getTime() < 86400000 * 2,
      ).length,
    };
  }, []);
        ...
        <Card className="border-border mb-6 bg-white py-0 shadow-sm">
          <CardContent className="flex flex-col items-center justify-between gap-4 p-4 sm:flex-row">
            ...
            <div className="flex w-full items-center gap-3 sm:w-auto">
              <span className="flex hidden items-center gap-1 text-xs font-bold whitespace-nowrap text-[#012a4a] sm:inline">
                                    ...
                                    {afterData ? (
                                      <div className="max-h-60 space-y-1 overflow-y-auto rounded-xl border border-emerald-100 bg-emerald-50/20 p-4 font-mono text-xs text-[#012a4a]">
                                        {Object.entries(afterData).map(([key, val]) => {
                                          // Simple change validation check
